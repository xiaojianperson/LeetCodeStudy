...
public:
    vector<int> findRightInterval(vector<vector<int>>& intervals) {
        ...
        for (int i = 0;i < intervals.size();i++) {
            vec1.emplace_back(intervals[i][0], i);
            vec2.emplace_back(intervals[i][1], i);
        }

        sort(vec1.begin(), vec1.end(), [](pair<int, int> p1, pair<int, int> p2) {
            return p1.first < p2.first;
            });

        sort(vec2.begin(), vec2.end(), [](pair<int, int> p1, pair<int, int> p2) {
            return p1.first < p2.first;
            });
        ...
        for (int i = 0;i < intervals.size();i++) {
            int left = start, right = intervals.size() - 1;
            int res = INT_MAX;
            int index = -1;
            ...
            while (left <= right) {
                ...
                if (vec1[mid].first > res) {
                    break;
                }

                if (vec2[i].first <= vec1[mid].first) {
                    if (vec1[mid].first < res) {
                        index = mid;
                        start = mid;
                        res = vec1[mid].first;
            ...
            int a = vec2[i].second;
            if (index != -1) {
                int b = vec1[index].second;
                ans[vec2[i].second] = vec1[index].second;
            }
            else {
                ans[vec2[i].second] = -1;
            }
...
int main() {
    ...
    solution.findRightInterval(arr);
    return 0;
}